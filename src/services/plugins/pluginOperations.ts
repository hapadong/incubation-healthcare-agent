...
/**
 * Is this plugin enabled (value === true) in .healthagent/settings.json?
 *
...
export async function uninstallPluginOp(
  ...
  if (!scopeInstallation) {
    ...
    if (actualScope !== scope && installations && installations.length > 0) {
      // Project scope is special: .healthagent/settings.json is shared with the team.
      // Point users at the local-override escape hatch instead of --scope project.
      if (actualScope === 'project') {
        return {
          success: false,
          message: `Plugin "${plugin}" is enabled at project scope (.healthagent/settings.json, shared with your team). To disable just for you: claude plugin disable ${plugin} --scope local`,
        }
      }
      return {
        success: false,
...
export async function setPluginEnabledOp(
  ...
  // without touching the shared .healthagent/settings.json).
...
async function performPluginUpdate({
  ...
  if (typeof entry.source !== 'string') {
    ...
    const manifestPath = join(sourcePath, '.healthagent-plugin', 'plugin.json')
    try {
