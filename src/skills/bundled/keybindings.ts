...
const FILE_FORMAT_EXAMPLE: KeybindingsSchemaType = {
  ...
  $docs: 'https://code.healthagent.com/docs/en/keybindings',
  bindings: [
    {
      context: 'Chat',
      bindings: {
...
const SECTION_INTRO = [
  '# Keybindings Skill',
  '',
  'Create or modify `~/.healthagent/keybindings.json` to customize keyboard shortcuts.',
  '',
  '## CRITICAL: Read Before Write',
  '',
  '**Always read `~/.healthagent/keybindings.json` first** (it may not exist yet). Merge changes with existing bindings — never replace the entire file.',
  '',
...
const SECTION_DOCTOR = [
  ...
  '',
  'The `/doctor` command includes a "Keybinding Configuration Issues" section that validates `~/.healthagent/keybindings.json`.',
  '',
  ...
  'Location: ~/.healthagent/keybindings.json',
...
export function registerKeybindingsSkill(): void {
  registerBundledSkill({
    ...
    description:
      'Use when the user wants to customize keyboard shortcuts, rebind keys, add chord bindings, or modify ~/.healthagent/keybindings.json. Examples: "rebind ctrl+s", "add a chord shortcut", "change the submit key", "customize keybindings".',
    allowedTools: ['Read'],
    userInvocable: false,
