...
|------|-------|-----|---------|
| \`~/.healthagent/settings.json\` | Global | N/A | Personal preferences for all projects |
| \`.healthagent/settings.json\` | Project | Commit | Team-wide hooks, permissions, plugins |
| \`.healthagent/settings.local.json\` | Project | Gitignore | Personal overrides for this project |
...
{
  "permissions": {
    "allow": ["Bash(npm:*)", "Edit(.healthagent)", "Read"],
    "deny": ["Bash(rm -rf:*)"],
    "ask": ["Write(/etc/*)"],
...
\`\`\`json
{
  "hooks": {
    "PreToolUse": [{
      ...
      "hooks": [{
        "type": "command",
        "command": "jq -r '.tool_input.command' >> ~/.healthagent/bash-log.txt"
      }]
    }]
  }
}
...
4. **Write the JSON.** Merge into the target file (schema shape in the "Hook Structure" section above). If this creates \`.healthagent/settings.local.json\` for the first time, add it to .gitignore — the Write tool doesn't auto-gitignore it.
...
6. **Prove the hook fires** — only for \`Pre|PostToolUse\` on a matcher you can trigger in-turn (\`Write|Edit\` via Edit, \`Bash\` via Bash). \`Stop\`/\`UserPromptSubmit\`/\`SessionStart\` fire outside this turn — skip to step 7.
   ...
   **If proof fails but pipe-test passed and \`jq -e\` passed**: the settings watcher isn't watching \`.healthagent/\` — it only watches directories that had a settings file when this session started. The hook is written correctly. Tell the user to open \`/hooks\` once (reloads config) or restart — you can't do this yourself; \`/hooks\` is a user UI menu and opening it ends this turn.
...
2. **Read**: \`.healthagent/settings.json\` (or create if missing)
...
1. **Check the settings file** - Read ~/.healthagent/settings.json or .healthagent/settings.json
