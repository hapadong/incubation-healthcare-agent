...
export function HelpV2(t0) {
  ...
  let t6;
  if ($[31] !== tabs) {
    t6 = <Tabs title={false ? "/help" : (process.env.HEALTHAGENT_API_BASE_URL ? `HealthAgent v${process.env.HEALTHAGENT_VERSION ?? '0.1.0'}` : `Claude Code v${'2.1.88'}`)} color="professionalBlue" defaultTab="general">{tabs}</Tabs>;
    $[31] = tabs;
    $[32] = t6;
  } else {
    t6 = $[32];
