...
export function getAllHooks(appState: AppState): IndividualHookConfig[] {
  ...
  if (!restrictedToManagedOnly) {
    ...
    // both resolve to ~/.healthagent/settings.json)
...
export function hookSourceDescriptionDisplayString(source: HookSource): string {
  switch (source) {
    case 'userSettings':
      return 'User settings (~/.healthagent/settings.json)'
    case 'projectSettings':
      return 'Project settings (.healthagent/settings.json)'
    case 'localSettings':
      return 'Local settings (.healthagent/settings.local.json)'
    case 'pluginHook':
      ...
      // e.g., "Plugin hooks (~/.healthagent/plugins/repos/source/example-plugin/example-plugin/hooks/hooks.json)"
      return 'Plugin hooks (~/.healthagent/plugins/*/hooks/hooks.json)'
    case 'sessionHook':
