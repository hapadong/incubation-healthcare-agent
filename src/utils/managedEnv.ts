 ...
 * env vars that the remote's ~/.healthagent settings.env MUST NOT clobber (see
 ...
 * user's ~/.healthagent/settings.json can't redirect requests away from the
 ...
 *
 * - userSettings (~/.healthagent/settings.json): controlled by the user, not project-specific
...
export function applySafeConfigEnvironmentVariables(): void {
  ...
  // Global config (~/.healthagent.json) is user-controlled. In CCD mode,
  ...
  // doesn't get clobbered by ~/.healthagent/settings.json env (gh#217). policy/flag
