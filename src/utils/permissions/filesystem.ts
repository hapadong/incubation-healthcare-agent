...
/**
 * If filePath is inside a .healthagent/skills/{name}/ directory (project or global),
 ...
 * require granting session access to all of .healthagent/ (settings.json, hooks/, etc.).
 */
export function getClaudeSkillScope(
  ...
  const bases = [
    {
      dir: expandPath(join(getOriginalCwd(), '.healthagent', 'skills')),
      prefix: '/.healthagent/skills/',
    },
    {
      dir: expandPath(join(getConfigHomeDir(), 'skills')),
      prefix: '~/.healthagent/skills/',
    },
  ]
  ...
  for (const { dir, prefix } of bases) {
    ...
    for (const s of [sep, '/']) {
      if (absolutePathLower.startsWith(dirLower + s.toLowerCase())) {
        ...
        // produce '/.healthagent/skills/*/**' which matches ALL skills. Return null
...
export function isClaudeSettingsPath(filePath: string): boolean {
  ...
  // sequences like `./.healthagent/./settings.json` which would evade the endsWith() check
  ...
  if (
    normalizedPath.endsWith(`${sep}.healthagent${sep}settings.json`) ||
    normalizedPath.endsWith(`${sep}.healthagent${sep}settings.local.json`)
  ) {
    // Include .healthagent/settings.json even for other projects
    return true
  }
...
function isClaudeConfigFilePath(filePath: string): boolean {
  ...
  // Check if file is within .healthagent/commands or .healthagent/agents directories
  // using proper path segment validation (not string matching with includes())
  // pathInWorkingPath now handles case-insensitive comparison to prevent bypasses
  const commandsDir = join(getOriginalCwd(), '.healthagent', 'commands')
  const agentsDir = join(getOriginalCwd(), '.healthagent', 'agents')
  const skillsDir = join(getOriginalCwd(), '.healthagent', 'skills')
 ...
 * Path format: ~/.healthagent/projects/{sanitized-cwd}/...
 */
...
function isDangerousFilePathToAutoEdit(path: string): boolean {
  ...
  for (let i = 0; i < pathSegments.length; i++) {
    ...
    for (const dir of DANGEROUS_DIRECTORIES) {
      ...
      // Special case: .healthagent/worktrees/ is a structural path (where Claude stores
      // git worktrees), not a user-created dangerous directory. Skip the .healthagent
      // segment when it's followed by 'worktrees'. Any nested .healthagent directories
      ...
      if (dir === '.healthagent') {
        ...
        if (
          ...
          break // Skip this .healthagent, continue checking other segments
        }
      }
 ...
 * - Trailing dots and spaces (e.g., .git., .healthagent , .bashrc...)
...
function hasSuspiciousWindowsPathPattern(path: string): boolean {
  ...
  // Examples: .git., .healthagent , .bashrc..., settings.json.
 ...
 * - Claude config files (.healthagent/settings.json, .healthagent/commands/, .healthagent/agents/)
...
export function checkPathSafetyForAutoEdit(
  ...
  for (const pathToCheck of pathsToCheck) {
    if (hasSuspiciousWindowsPathPattern(pathToCheck)) {
      return {
        safe: false,
        message: `Verity requested permissions to write to ${path}, which contains a suspicious Windows path pattern that requires manual approval.`,
  ...
  for (const pathToCheck of pathsToCheck) {
    if (isClaudeConfigFilePath(pathToCheck)) {
      return {
        safe: false,
        message: `Verity requested permissions to write to ${path}, but you haven't granted it yet.`,
  ...
  for (const pathToCheck of pathsToCheck) {
    if (isDangerousFilePathToAutoEdit(pathToCheck)) {
      return {
        safe: false,
        message: `Verity requested permissions to edit ${path} which is a sensitive file.`,
        classifierApprovable: true,
      }
    }
  }
...
function patternWithRoot(
  ...
  if (pattern.startsWith(`${DIR_SEP}${DIR_SEP}`)) {
    ...
    // Patterns starting with / resolve relative to the directory where settings are stored (without .healthagent/)
    return {
...
export function checkReadPermissionForTool(
  ...
  if (typeof tool.getPath !== 'function') {
    return {
      behavior: 'ask',
      message: `Verity requested permissions to use ${tool.name}, but you haven't granted it yet.`,
    }
  }
  ...
  for (const pathToCheck of pathsToCheck) {
    if (pathToCheck.startsWith('\\\\') || pathToCheck.startsWith('//')) {
      return {
        behavior: 'ask',
        message: `Verity requested permissions to read from ${path}, which appears to be a UNC path that could access network resources.`,
        decisionReason: {
          type: 'other',
  ...
  for (const pathToCheck of pathsToCheck) {
    if (hasSuspiciousWindowsPathPattern(pathToCheck)) {
      return {
        behavior: 'ask',
        message: `Verity requested permissions to read from ${path}, which contains a suspicious Windows path pattern that requires manual approval.`,
        decisionReason: {
          type: 'other',
          reason:
  ...
  for (const pathToCheck of pathsToCheck) {
    ...
    if (askRule) {
      return {
        behavior: 'ask',
        message: `Verity requested permissions to read from ${path}, but you haven't granted it yet.`,
        decisionReason: {
          type: 'rule',
          rule: askRule,
        },
...
export function checkWritePermissionForTool<Input extends AnyObject>(
  ...
  // This MUST come before isDangerousFilePathToAutoEdit check since .healthagent is a dangerous directory
  ...
  // 1.6. Check for .healthagent/** allow rules BEFORE safety checks
  // This allows session-level permissions to bypass the safety blocks for .healthagent/
  // We only allow this for session-level rules to prevent users from accidentally
  // permanently granting broad access to their .healthagent/ folder.
  //
  // matchingRuleForInput returns the first match across all sources. If the user
  // also has a broader Edit(.healthagent) rule in userSettings (e.g. from sandbox
  ...
  if (claudeFolderAllowRule) {
    // Check if this rule is scoped under .healthagent/ (project or global).
    // Accepts both the broad patterns ('/.healthagent/**', '~/.healthagent/**') and
    // narrowed ones like '/.healthagent/skills/my-skill/**' so users can grant
    ...
    // '/.healthagent/../**' from leaking this bypass outside .healthagent/.
  ...
  if (!safetyCheck.safe) {
    // SDK suggestion: if under .healthagent/skills/{name}/, emit the narrowed
    // session-scoped addRules that step 1.6 will honor on the next call.
    // Everything else (.healthagent/settings.json, .git/, .vscode/, .idea/) falls
...
export function checkEditableInternalPath(
  ...
  // var value must itself resolve under ~/.healthagent/jobs/. Symlink guard: every
  ...
  if (feature('TEMPLATES')) {
    ...
    if (jobDir) {
      ...
      // the case where ~/.healthagent is a symlink (e.g. to /data/claude-config).
  ...
  // ~/.healthagent/, which is in DANGEROUS_DIRECTORIES. The CLAUDE_COWORK_MEMORY_PATH_OVERRIDE
  ...
  // .healthagent/launch.json — desktop preview config (dev server command + port).
  ...
  // .healthagent/ DANGEROUS_DIRECTORIES check prompts for it, which in SDK mode
  ...
  // .healthagent/ only (not ~/.healthagent/) since launch.json is per-project.
  if (
    normalizeCaseForComparison(normalizedPath) ===
    normalizeCaseForComparison(join(getOriginalCwd(), '.healthagent', 'launch.json'))
  ) {
    return {
      behavior: 'allow',
      updatedInput: input,
...
export function checkReadableInternalPath(
  ...
  // Tasks directory (~/.healthagent/tasks/) for swarm task coordination
  const tasksDir = join(getConfigHomeDir(), 'tasks') + sep
  if (
  ...
  // Teams directory (~/.healthagent/teams/) for swarm coordination
  const teamsReadDir = join(getConfigHomeDir(), 'teams') + sep
  if (
