 ...
 * user-controlled files (~/.healthagent/settings.json and ~/.healthagent.json),
 ...
 *
 * We read from global config (~/.healthagent.json) and user settings
 * (~/.healthagent/settings.json). These are user-controlled files that don't
...
function getExtraCertsPathFromConfig(): string | undefined {
  try {
    ...
    // Only read from user-controlled settings (~/.healthagent/settings.json),
