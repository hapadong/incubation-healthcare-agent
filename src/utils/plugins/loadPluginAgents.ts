...
async function loadAgentFromFile(
  ...
  try {
    ...
    // time. For this level of control, define the agent in .healthagent/agents/
    ...
    for (const field of ['permissionMode', 'hooks', 'mcpServers'] as const) {
      if (frontmatter[field] !== undefined) {
        logForDebugging(
          `Plugin agent file ${filePath} sets ${field}, which is ignored for plugin agents. Use .healthagent/agents/ for this level of control.`,
          { level: 'warn' },
        )
      }
    }
