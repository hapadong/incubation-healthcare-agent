 ...
 *
 * State persists in GlobalConfig.healthagentCodeHints — a show-once record per
...
export function maybeRecordPluginHint(hint: ClaudeCodeHint): void {
  ...
  const state = getGlobalConfig().healthagentCodeHints
...
export function markHintPluginShown(pluginId: string): void {
  saveGlobalConfig(current => {
    const existing = current.healthagentCodeHints?.plugin ?? []
    if (existing.includes(pluginId)) return current
    return {
      ...current,
      claudeCodeHints: {
        ...current.healthagentCodeHints,
...
export function disableHintRecommendations(): void {
  saveGlobalConfig(current => {
    if (current.healthagentCodeHints?.disabled) return current
    return {
      ...current,
      claudeCodeHints: { ...current.healthagentCodeHints, disabled: true },
    }
  })
}
