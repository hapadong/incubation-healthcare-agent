 ...
 * Format: ~/.healthagent/plugins/cache/{marketplace}/{plugin}/{version}/
 *
 * @param pluginId - Plugin identifier in format "name@marketplace"
 * @param version - Version string (semver, git SHA, etc.)
 ...
 * Format: ~/.healthagent/plugins/cache/{plugin-name}/
 *
...
export async function cachePlugin(
  ...
  const manifestPath = join(tempPath, '.healthagent-plugin', 'plugin.json')
 ...
 * @param source - Source description for default manifest (e.g., "git:repo" or ".healthagent-plugin/name")
 ...
 * @param source - Source identifier (e.g., "git:repo", ".healthagent-plugin/my-plugin")
...
export async function createPluginFromPath(
  ...
  const manifestPath = join(pluginPath, '.healthagent-plugin', 'plugin.json')
  ...
  const plugin: LoadedPlugin = {
    ...
    source, // Source identifier (e.g., "git:repo" or ".healthagent-plugin/name")
 ...
 * - After modifying .healthagent-plugin/ directory (for export)
