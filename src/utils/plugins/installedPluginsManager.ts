 ...
 *
 * The enabled/disabled state remains in .healthagent/settings.json for per-repo control.
 *
 ...
 *
 * Legacy cache structure: ~/.healthagent/plugins/cache/{plugin-name}/
 * Versioned cache structure: ~/.healthagent/plugins/cache/{marketplace}/{plugin}/{version}/
 *
...
function migrateV1ToV2(v1Data: InstalledPluginsFileV1): InstalledPluginsFileV2 {
  ...
  for (const [pluginId, plugin] of Object.entries(v1Data.plugins)) {
    // V2 format uses versioned cache path: ~/.healthagent/plugins/cache/{marketplace}/{plugin}/{version}
...
function getPluginVersionFromManifest(
  ...
  const fs = getFsImplementation()
  const manifestPath = join(pluginCachePath, '.healthagent-plugin', 'plugin.json')
    ...
    if (existingInstallations && existingInstallations.length > 0) {
      ...
      try {
        ...
        if (typeof entry.source === 'string') {
          ...
          // Only read manifest if the .healthagent-plugin dir is present
          if (dirEntries.includes('.healthagent-plugin')) {
