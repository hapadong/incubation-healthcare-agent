...
function detectManifestType(
  ...
  // Check if it's in .healthagent-plugin directory
  if (dirName === '.healthagent-plugin') {
 ...
 * '..' a user added to "climb out of .healthagent-plugin/" is unnecessary. Callers pass `hint`
...
// they expect paths to resolve relative to marketplace.json (inside .healthagent-plugin/),
...
function marketplaceSourceHint(p: string): string {
  ...
  // .healthagent-plugin/" is unnecessary since paths already start at the repo root.
  ...
  return (
    ...
    'containing .healthagent-plugin/), not relative to marketplace.json. ' +
  ...
  if (result.success) {
    ...
    if (marketplace.plugins) {
      ...
      const marketplaceRoot =
        path.basename(manifestDir) === '.healthagent-plugin'
      ...
      for (const [i, entry] of marketplace.plugins.entries()) {
        ...
        if (manifestVersion && manifestVersion !== entry.version) {
          warnings.push({
            ...
            message:
              `Entry declares version "${entry.version}" but ${entry.source}/.healthagent-plugin/plugin.json says "${manifestVersion}". ` +
...
export async function validateManifest(
  ...
  if (stats?.isDirectory()) {
    // Look for manifest files in .healthagent-plugin directory
    ...
    const pluginPath = path.join(absolutePath, '.healthagent-plugin', 'plugin.json')
    ...
    return {
      ...
      errors: [
        {
          path: 'directory',
          message: `No manifest found in directory. Expected .healthagent-plugin/marketplace.json or .healthagent-plugin/plugin.json`,
        },
      ],
      warnings: [],
      filePath: absolutePath,
