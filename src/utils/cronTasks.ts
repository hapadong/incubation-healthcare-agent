// Scheduled prompts, stored in <project>/.healthagent/scheduled_tasks.json.
//
...
const CRON_FILE_REL = join('.healthagent', 'scheduled_tasks.json')
...
/**
 * Read and parse .healthagent/scheduled_tasks.json. Returns an empty task list if the file
...
/**
 * Overwrite .healthagent/scheduled_tasks.json with the given tasks. Creates .healthagent/ if
...
export async function writeCronTasks(
  ...
  await mkdir(join(root, '.healthagent'), { recursive: true })
 ...
 * written to .healthagent/scheduled_tasks.json and dies with the process. The
