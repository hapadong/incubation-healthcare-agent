 ...
 * 2. User memory (~/.healthagent/CLAUDE.md) - Private global instructions for all projects
 * 3. Project memory (CLAUDE.md, .healthagent/CLAUDE.md, and .healthagent/rules/*.md in project roots) - Instructions checked into the codebase
 ...
 * - CLAUDE.md, .healthagent/CLAUDE.md, and all .md files in .healthagent/rules/ are checked in each directory for Project memory
 *
...
function isClaudeMdExcluded(filePath: string, type: MemoryType): boolean {
  ...
  const patterns = getInitialSettings().healthagentMdExcludes
...
/**
 * Processes all .md files in the .healthagent/rules/ directory and its subdirectories
...
export const getMemoryFiles = memoize(
  async (forceIncludeExternal: boolean = false): Promise<MemoryFileInfo[]> => {
    ...
    // Process Managed .healthagent/rules/*.md files
    ...
    if (isSettingSourceEnabled('userSettings')) {
      ...
      // Process User ~/.healthagent/rules/*.md files
    ...
    // .healthagent/worktrees/<name>/ from `claude -w`), the upward walk passes
    // through both the worktree root and the main repo root. Both contain
    // checked-in files like CLAUDE.md and .healthagent/rules/*.md, so the same
    ...
    for (const dir of dirs.reverse()) {
      ...
      if (isSettingSourceEnabled('projectSettings') && !skipProject) {
        ...
        // Try reading .healthagent/CLAUDE.md (Project)
        const dotClaudePath = join(dir, '.healthagent', 'CLAUDE.md')
        result.push(
          ...(await processMemoryFile(
            dotClaudePath,
            'Project',
        ...
        // Try reading .healthagent/rules/*.md files (Project)
        const rulesDir = join(dir, '.healthagent', 'rules')
        result.push(
          ...(await processMdRules({
            rulesDir,
            type: 'Project',
    ...
    if (isEnvTruthy(process.env.CLAUDE_CODE_ADDITIONAL_DIRECTORIES_CLAUDE_MD)) {
      ...
      for (const dir of additionalDirs) {
        ...
        // Try reading .healthagent/CLAUDE.md from the additional directory
        const dotClaudePath = join(dir, '.healthagent', 'CLAUDE.md')
        result.push(
          ...(await processMemoryFile(
            dotClaudePath,
        ...
        // Try reading .healthagent/rules/*.md files from the additional directory
        const rulesDir = join(dir, '.healthagent', 'rules')
        result.push(
          ...(await processMdRules({
            rulesDir,
...
export async function getManagedAndUserConditionalRules(
  ...
  // Process Managed conditional .healthagent/rules/*.md files
  const managedClaudeRulesDir = getManagedClaudeRulesDir()
  result.push(
  ...
  if (isSettingSourceEnabled('userSettings')) {
    // Process User conditional .healthagent/rules/*.md files
    const userClaudeRulesDir = getUserClaudeRulesDir()
    result.push(
      ...(await processConditionedMdRules(
        targetPath,
...
export async function getMemoryFilesForNestedDirectory(
  ...
  // Process project memory files (CLAUDE.md and .healthagent/CLAUDE.md)
  ...
  // Process project unconditional .healthagent/rules/*.md files, which were not eagerly loaded
  ...
  // Process project conditional .healthagent/rules/*.md files
  result.push(
    ...(await processConditionedMdRules(
      targetPath,
      rulesDir,
...
export async function processConditionedMdRules(
  ...
  return conditionedRuleMdFiles.filter(file => {
    ...
    // For Project rules: glob patterns are relative to the directory containing .healthagent
    // For Managed/User rules: glob patterns are relative to the original CWD
    const baseDir =
      type === 'Project'
        ? dirname(dirname(rulesDir)) // Parent of .healthagent
...
/**
 * Check if a file path is a memory file (CLAUDE.md, CLAUDE.local.md, or .healthagent/rules/*.md)
 */
export function isMemoryFilePath(filePath: string): boolean {
  ...
  // .md files in .healthagent/rules/ directories
  if (
    name.endsWith('.md') &&
    filePath.includes(`${sep}.healthagent${sep}rules${sep}`)
  ) {
    return true
  }
