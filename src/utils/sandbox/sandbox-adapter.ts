...
export function convertToSandboxRuntimeConfig(
  ...
  if (cwd !== originalCwd) {
    denyWrite.push(resolve(cwd, '.healthagent', 'settings.json'))
    denyWrite.push(resolve(cwd, '.healthagent', 'settings.local.json'))
  }

  // Block writes to .healthagent/skills in both original and current working directories.
  // The sandbox-runtime's getDangerousDirectories() protects .healthagent/commands and
  // .healthagent/agents but not .healthagent/skills. Skills have the same privilege level
  // (auto-discovered, auto-loaded, full Claude capabilities) so they need the
  // same OS-level sandbox protection.
  denyWrite.push(resolve(originalCwd, '.healthagent', 'skills'))
  if (cwd !== originalCwd) {
    denyWrite.push(resolve(cwd, '.healthagent', 'skills'))
  }
