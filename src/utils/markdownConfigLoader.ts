...
export type ClaudeConfigDirectory = (typeof CONFIG_DIRECTORIES)[number]
export type ConfigDirectory = ClaudeConfigDirectory
 ...
 * stopping there makes the parent project's `.healthagent/` unreachable (#31905).
 *
 ...
 *
 * Worktrees (under `.healthagent/worktrees/`) stay on the old behavior: their `.git`
 ...
 * collecting all .healthagent directories along the way.
 *
 * Stopping at git root prevents commands/skills from parent directories outside the repository
 * from leaking into projects. For example, if ~/projects/.healthagent/commands/ exists, it won't
 ...
 * @returns Array of directory paths containing .healthagent/subdir, from most specific (cwd) to least specific
 */
export function getProjectDirsUpToHome(
  ...
  while (true) {
    ...
    const claudeSubdir = join(current, '.healthagent', subdir)
...
export const loadMarkdownFilesForSubdir = memoize(
  async function (
    ...
    const managedDir = join(getManagedFilePath(), '.healthagent', subdir)
    const projectDirs = getProjectDirsUpToHome(subdir, cwd)

    // For git worktrees where the worktree does NOT have .healthagent/<subdir> checked
    ...
    //
    // Only add the main repo's copy when the worktree root's .healthagent/<subdir>
    // is absent. A standard `git worktree add` checks out the full tree, so the
    // worktree already has identical .healthagent/<subdir> content — loading the main
    ...
    // This prevents the same file from appearing multiple times when ~/.healthagent is
 ...
 * @param dir Directory (eg. "~/.healthagent/commands")
 * @returns Array of parsed markdown files with metadata
 */
