// Scheduler lease lock for .healthagent/scheduled_tasks.json.
//
...
const LOCK_FILE_REL = join('.healthagent', 'scheduled_tasks.lock')
...
async function tryCreateExclusive(
  ...
  try {
    ...
    if (code === 'ENOENT') {
      // .healthagent/ doesn't exist yet — create it and retry once. In steady
