...
function isPathAllowed(
  ...
  // This MUST come before checkPathSafetyForAutoEdit since .healthagent is a dangerous directory
  // and internal editable paths live under ~/.healthagent/ — matching the ordering in
...
function checkPathConstraintsForStatement(
  ...
  //   Set-Location ./.healthagent; Set-Content ./settings.json '...'
  // Validator sees ./settings.json → /project/settings.json (not a config file).
  // Runtime writes /project/.healthagent/settings.json (Claude's permission config).
  //
  // ALTERNATIVE APPROACH (rejected): simulate cwd through the statement chain
  // — after `Set-Location ./.healthagent`, validate subsequent statements with
  // cwd='./.healthagent'. This would be more permissive but requires careful
  // handling of:
