...
const inputSchema = lazySchema(() =>
  z.strictObject({
    ...
    durable: semanticBoolean(z.boolean().optional()).describe(
      'true = persist to .healthagent/scheduled_tasks.json and survive restarts. false (default) = in-memory only, dies when this Claude session ends. Use true only when the user asks the task to survive across sessions.',
    ),
  }),
)
...
export const CronCreateTool = buildTool({
  ...
  mapToolResultToToolResultBlockParam(output, toolUseID) {
    const where = output.durable
      ? 'Persisted to .healthagent/scheduled_tasks.json'
